...
- **Severe overlay stacking fix**: severe polygons now render in explicit severity order so higher-probability areas sit above lower ones, and when probabilities tie we draw `SIGN` last (on top) to preserve the “significant” emphasis SPC intends.
...
- **Concentric radius pass (Liquid Glass fidelity)**: we replaced scattered hardcoded corner radii with a shared `SkyAwareRadius` scale (`hero/card/section/content/row/tile/chip`) and updated view surfaces/chips/buttons to consume those tokens. This keeps nested shapes visually concentric and prevents subtle “almost matching” corners that make glass UIs feel off.
...
- **WeatherKit throttle pass**: we mirrored the Outlook sync policy pattern for WeatherKit (`WeatherKitRefreshPolicy`) and now only refresh current conditions every 30 minutes unless the user forces a refresh. This cut repeated WeatherKit calls during active-location churn while keeping manual refresh behavior immediate.
- **Bug squash (refresh cancellation noise)**: manual/interactive refresh can legitimately cancel in-flight SPC text sync work; we now treat `CancellationError` as expected control flow in convective + meso sync (notice-level), instead of logging it as a failure. This keeps logs honest and makes real sync failures easier to spot.
...
- **CIG hatching pass (procedural, not sticker art)**: map overlays now support typed risk overlays (`probability` vs `intensity(level:)`) so intensity texture can be controlled independently from base fills. `MapCanvasView` gained an overlay adapter path (`MKMultiPolygon` still works, but we can now feed `[MKOverlay]` directly), and overlay diffing now hashes style + geometry so restyles don’t silently fail.
- **War story (the “warning tape” trap)**: first-pass hatch prototypes looked like black construction stripes on top of the basemap. We switched to airy defaults (14pt spacing, ~0.24 opacity), level-based spacing tweaks (CIG1 looser, CIG3 denser), and dark-mode near-white tinting so texture reads as context instead of visual shouting.
- **Aha! (MapKit renderer math)**: crisp hatching across zoom levels means drawing in renderer space and scaling spacing/line width by `1 / zoomScale`. The gating check became a simple `zoomScale` threshold, so zoomed-out views stay clean while regional+ zoom reveals texture detail.
