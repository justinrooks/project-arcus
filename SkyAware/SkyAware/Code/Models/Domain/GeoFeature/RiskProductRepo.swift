...
@ModelActor
actor RiskProductRepo {
    private let logger = Logger.riskProductRepo
    ...
    func refreshStormRisk(using client: any SpcClient) async throws {
        ...
        let dtos = decoded.features.compactMap {
            ...
            return RiskFeature(categorical: StormRiskLevel(abbreviation: props.LABEL),
        ...
        try upsert(dtos)
        logger.debug("Updated \(dtos.count) categorical storm risk product\(dtos.count > 1 ? "s" : "")")
    }
...
//         3) For each risk, check polygons with optional bbox prefilter, then precise hit test
    ...
    private func upsert(_ items: [RiskFeature]) throws {
        for item in items {
