...
### Overview
This build makes Storm Setup eligibility more precise and hardens recurring background refreshes with bounded work, cooperative cancellation, durable context reuse, and clearer scheduling diagnostics.

### Highlights
- Storm Setup now presents only for active qualifying severe-thunderstorm or tornado watches and warnings.
...
- Background refresh can reuse durable location and NWS region context when the cached context remains valid.
- Regression coverage was expanded across Storm Setup eligibility, background lifecycle and cadence, cancellation, deadline-aware networking, location-context reuse, and ingestion coordination.
...
### Overview
This build adds Home widgets for storm and severe-risk snapshots, with refreshed snapshot writes and reload coordination to keep widgets current.

### Highlights
- Add Home widgets (small and large) that surface storm and severe-risk snapshots with tap-through routing into the app.
- Widget snapshots are now generated and persisted during Home refresh, with widget reloads coordinated on refresh and hot-alert events.
- Automated coverage now includes widget snapshot modeling/storage, route URL handling, and freshness formatting.
...
- Cancelled and inactive Arcus watch payloads are now filtered out before they reach local watch results.
...
- Duplicate location-scoped Arcus and mesoscale syncs now join in-flight work instead of refetching the same local data.
- Foreground and background ingestion now share the same hot-feed pipeline, while slower SPC feeds are refreshed separately.

### UI / UX
- Onboarding permission copy now explains the Always Allow follow-up request and how location is used to determine local weather alerts.
- Summary loading now presents more detailed status messaging while local context, alerts, and conditions are resolving.

### Tests / QA
- Added regression coverage for deterministic location-context readiness, Home refresh pipeline behavior, watch cancellation/update handling, and background location-change watch notifications.
