...
struct SummaryViewStormSetupSlotStateTests {
    @Test("composed Storm Setup state reserves every enabled section state")
    func composedStateAndPlanRespectSlotContract() {
        let cases: [(String, SummaryView.StormSetupSlotState, Bool)] = [
            ...
            ("no notable setup", .status(.noNotableSetup), true),
            ("analysis not needed", .status(.analysisNotNeeded), true),
            ("unavailable", .status(.unavailable), true),
            ("hidden", .hidden, false)
        ]

        for (_, slot, shouldInclude) in cases {
