...
struct HomeRefreshPipelineTests {
    ...
    @Test("visible commit propagates profile analysis and keeps existing home slices intact")
    func visibleCommit_propagatesProfileAnalysisAndKeepsOtherSlicesIntact() async {
        let context = makeContext()
        let sampleMd = MD.sampleDiscussionDTOs[0]
        let sampleAlerts = [Watch.sampleWatchRows[0]]
        let sampleOutlooksValue = sampleOutlooks()
        ...
        let profileAnalysis = makeProfileAnalysisPayload(
            response: makeProfileAnalysisResponse()
        )
        let snapshot = HomeSnapshot(
            locationSnapshot: context.snapshot,
            refreshKey: context.refreshKey,
            ...
            stormSetup: stormSetup,
            stormSetupRefreshResult: .success,
            stormSetupProfileAnalysisPayload: profileAnalysis,
            stormSetupProfileAnalysisRefreshResult: .success,
            stormRisk: .moderate,
            severeRisk: .hail(probability: 0.25),
            fireRisk: .elevated,
            mesos: [sampleMd],
        ...
        #expect(pipeline.stormSetup == stormSetup)
        #expect(pipeline.stormSetupRefreshKey == context.refreshKey)
        #expect(pipeline.stormSetupProfileAnalysisPayload == profileAnalysis)
        #expect(pipeline.stormSetupProfileAnalysisRefreshKey == context.refreshKey)
        #expect(pipeline.stormRisk == .moderate)
        #expect(pipeline.severeRisk == .hail(probability: 0.25))
        #expect(pipeline.fireRisk == .elevated)
        #expect(pipeline.mesos == [sampleMd])
    ...
    @Test("same-location nil or failed profile analysis preserves the existing value")
    func sameLocationNilOrFailurePreservesExistingProfileAnalysis() async {
        let context = makeContext()
        let existingProfileAnalysis = makeProfileAnalysisPayload()
        let pipeline = HomeRefreshPipeline(
            initialStormSetupProfileAnalysisPayload: existingProfileAnalysis,
            initialStormSetupProfileAnalysisRefreshKey: context.refreshKey
        )
        let locationSession = FakeLocationSession(currentContext: context, preparedContext: context)
        let nilSnapshot = HomeSnapshot(
            locationSnapshot: context.snapshot,
            ...
            stormSetupProfileAnalysisRefreshResult: .timeout
        )
        let failureCoordinator = RecordingHomeIngestionCoordinator(snapshot: nilSnapshot)
        ...
        #expect(pipeline.stormSetupProfileAnalysisPayload == existingProfileAnalysis)
        #expect(pipeline.stormSetupProfileAnalysisRefreshKey == context.refreshKey)
        ...
        let failureCoordinator2 = RecordingHomeIngestionCoordinator(
            snapshot: HomeSnapshot(
                ...
                refreshKey: context.refreshKey,
                stormSetupProfileAnalysisPayload: nil,
                stormSetupProfileAnalysisRefreshResult: .failure
            )
        )
    ...
    @Test("different-location nil profile analysis clears the previous value")
    func differentLocationNilProfileAnalysisClearsPreviousValue() async {
        let oldContext = makeContext(h3Cell: 111_111)
        let newContext = makeContext(h3Cell: 222_222, timestamp: 200)
        let existingProfileAnalysis = makeProfileAnalysisPayload()
        let pipeline = HomeRefreshPipeline(
            initialStormSetupProfileAnalysisPayload: existingProfileAnalysis,
            initialStormSetupProfileAnalysisRefreshKey: oldContext.refreshKey
        )
        let coordinator = RecordingHomeIngestionCoordinator(
            snapshot: HomeSnapshot(
                locationSnapshot: newContext.snapshot,
        ...
        #expect(pipeline.stormSetupProfileAnalysisPayload == nil)
        #expect(pipeline.stormSetupProfileAnalysisRefreshKey == newContext.refreshKey)
    }

    @Test("visible matching profile analysis replaces the older value")
    func visibleMatchingProfileAnalysisReplacesOlderValue() async {
        let context = makeContext()
        let oldProfileAnalysis = makeProfileAnalysisPayload(
            response: makeProfileAnalysisResponse(mlcape: 1_500)
        )
        let newProfileAnalysis = makeProfileAnalysisPayload(
            response: makeProfileAnalysisResponse(mlcape: 2_200)
        )
        let pipeline = HomeRefreshPipeline(
            initialStormSetupProfileAnalysisPayload: oldProfileAnalysis,
            initialStormSetupProfileAnalysisRefreshKey: context.refreshKey
        )
        let coordinator = RecordingHomeIngestionCoordinator(
            snapshot: HomeSnapshot(
                locationSnapshot: context.snapshot,
                refreshKey: context.refreshKey,
                stormSetupProfileAnalysisPayload: newProfileAnalysis,
                stormSetupProfileAnalysisRefreshResult: .success
            )
        )
        let locationSession = FakeLocationSession(currentContext: context, preparedContext: context)
        ...
        #expect(pipeline.stormSetupProfileAnalysisPayload == newProfileAnalysis)
        #expect(pipeline.stormSetupProfileAnalysisRefreshKey == context.refreshKey)
    }

    @Test("prime snapshot keeps old profile analysis ownership until a visible commit lands")
    func primeSnapshot_keepsOldProfileAnalysisOwnershipUntilVisibleCommit() async {
        let oldContext = makeContext(h3Cell: 111_111)
        let newContext = makeContext(h3Cell: 222_222, timestamp: 200)
        let oldProfileAnalysis = makeProfileAnalysisPayload()
        let primeProfileAnalysis = makeProfileAnalysisPayload(
            response: makeProfileAnalysisResponse(mlcape: 2_000)
        )
        let primeSnapshot = HomeSnapshot(
            locationSnapshot: newContext.snapshot,
            refreshKey: newContext.refreshKey,
            stormSetupProfileAnalysisPayload: primeProfileAnalysis,
            stormSetupProfileAnalysisRefreshResult: .success
        )
        let followUpGate = AsyncGate()
        let followUpSnapshot = HomeSnapshot(
        ...
        #expect(requestCountReached)
        #expect(pipeline.lastResolvedLocationScopedRefreshKey == newContext.refreshKey)
        #expect(pipeline.stormSetupProfileAnalysisPayload == oldProfileAnalysis)
        #expect(pipeline.stormSetupProfileAnalysisRefreshKey == oldContext.refreshKey)
