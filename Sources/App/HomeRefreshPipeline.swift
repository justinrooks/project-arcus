...
final class HomeRefreshPipeline {
    ...
    private var didCompleteConditionsRefreshSuccessfully = false
    ...
    private func submit(_ trigger: HomeView.RefreshTrigger, waitsForCompletion: Bool) async {
        ...
        if waitsForCompletion {
            ...
            finishForegroundRefresh(outcome: outcome)
            if trigger == .manual {
        ...
        Task { @MainActor [weak self] in
            ...
            self.finishForegroundRefresh(outcome: outcome)
            if trigger == .manual {
    ...
    private func runRefresh(
        ...
        do {
            ...
            return refreshOutcome(for: snapshot, environment: environment)
    ...
    private func refreshOutcome(for snapshot: HomeSnapshot, environment: Environment) -> RefreshOutcome {
    ...
    private func beginForegroundRefresh() {
        if activeRefreshCount == 0, followUpRefreshCount == 0 {
            didCompleteConditionsRefreshSuccessfully = false
        }
    ...
    private func finishForegroundRefresh(outcome: RefreshOutcome) {
        guard activeRefreshCount > 0 else { return }
        if outcome == .completed {
            didCompleteConditionsRefreshSuccessfully = true
        }
        activeRefreshCount -= 1
    ...
    private func scheduleFollowUpRefresh(
        ...
        Task { @MainActor [weak self] in
            ...
            defer {
                if outcome == .completed {
                    self.didCompleteConditionsRefreshSuccessfully = true
                }
    ...
    private func finalizeForegroundRefreshIfNeeded() {
        ...
        resolutionState.finishAll(
            ...
            conditionsUpdated: didCompleteConditionsRefreshSuccessfully
        )
        didCompleteConditionsRefreshSuccessfully = false
