//
//  StormSetupProfileAnalysisClient.swift
//  SkyAware
//
...
struct StormSetupProfileAnalysisHTTPClient: StormSetupProfileAnalysisQuerying {
    ...
    func fetchProfileAnalysis(h3Cell: Int64) async throws -> StormSetupProfileAnalysisDTO {
        let url = try makeURL(
            queryItems: [
                URLQueryItem(name: "h3", value: String(h3Cell))
            ]
        )
        logger.info(
            "Arcus request started endpoint=\(url.path, privacy: .public) mode=\(HTTPExecutionMode.current.logName, privacy: .public) request=profile-analysis"
        )
    ...
    private func fetch(from url: URL) async throws -> StormSetupProfileAnalysisDTO {
        do {
            ...
            case .success, .notModified:
                ...
                do {
                    let dto = try DecoderFactory.iso8601.decode(StormSetupProfileAnalysisDTO.self, from: data)
                    logger.info(
    ...
    private func makeURL(queryItems: [URLQueryItem]) throws -> URL {
        guard let url = ArcusSignalConfiguration.url(
            from: baseURL,
            path: ArcusSignalConfiguration.stormSetupProfileAnalysisPath,
